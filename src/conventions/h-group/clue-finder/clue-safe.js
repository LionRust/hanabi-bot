...
export function clue_safe(game, player, clue) {
	...
	let hypo_game = game.simulate_clue(clue_action);
	hypo_game.catchup = true;

	// Update waiting connections
	hypo_game.last_actions[state.ourPlayerIndex] = clue_action;
	hypo_game = hypo_game.handle_action({ type: 'turn', num: state.turn_count, currentPlayerIndex: state.nextPlayerIndex(state.ourPlayerIndex) });
